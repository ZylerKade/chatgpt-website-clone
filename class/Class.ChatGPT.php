...
// Used for setting up a sensitive word filter if activated
...
// Used to start the conversation with the model
    public function qa($params){
		...
		if(empty($this->model)){
            $this->streamHandler->end('No model has been selected');
            return;
        }
...
// Sends the request to the OpenAI API, handles the response, and logs any cURL errors that occur
